...
def test_train_self_prompt_script_can_disable_proposal_summary(tmp_path):
    config_path = tmp_path / "self_prompt_synthetic_no_summary.yaml"
    checkpoint_dir = tmp_path / "checkpoints"
    config_path.write_text(
        f"""
synthetic: true
...
""",
    ...
    assert "epoch=1" in result.stdout
    assert "proposal_summary" not in result.stdout
    assert (checkpoint_dir / "best.pt").exists()
